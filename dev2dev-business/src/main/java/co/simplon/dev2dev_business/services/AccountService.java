...
public class AccountService {
	...
	public Map<String, Boolean> updateAccountNotificationType(Map<String, Boolean> userSettings) {
		...
		Account account = accountsRepo.findByUsernameIgnoreCase(accountEmail)
				.orElseThrow(() -> new BadCredentialsException(
						String.format("Account with username: %s not found !", accountEmail)));
		...
		userSettings.keySet().forEach(key -> {
			if (userSettings.get(key)) {
				account.addNotificationType(
						notificationTypeList.stream().filter(n -> n.getTypeName().equals(key)).toList().getFirst());
			} else {
				account.removeNotificationType(
						notificationTypeList.stream().filter(n -> n.getTypeName().equals(key)).toList().getFirst());
			}
		});
		Account updatedAccount = accountsRepo.saveAndFlush(account);
		Map<String, Boolean> notificationAccountSetting = new HashMap<>();
		notificationTypeList.forEach(notificationType -> notificationAccountSetting.put(notificationType.getTypeName(),
				updatedAccount.getNotificationTypeSet().stream().map(NotificationType::getTypeName).toList()
	...
	public Map<String, Boolean> getAccountNotificationType() {
		...
		Map<String, Boolean> notificationAccountSetting = new HashMap<>();
		notificationTypeSet.forEach(notificationType -> notificationAccountSetting.put(notificationType.getTypeName(),
				account.getNotificationTypeSet().stream().map(NotificationType::getTypeName).toList()
						.contains(notificationType.getTypeName())));
		return notificationAccountSetting;
	}
