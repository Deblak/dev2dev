...
3.2 As 'superuser' (postgres by default) run the script:
```DB_role_init.sql```
```DB_role_api.sql```

3.3 Login as "dbinit" and run the script:
```schema.ddl.sql```
to create the tables.

3.4 then login as "api" and run the scipt:
```data.dml.sql```
to insert data.

3.5. Update Backend Application Configuration
